...
Linux also supports syscall filtering via \texttt{seccomp-bpf} filters, which are used in conjunction with other container primitives to implement containerization in Docker, LXC, etc. Currently, the container runtime \textit{runC} (built on its predecessor \textit{libcontainer} \cite{hykes2014libcontainer}) uses \texttt{seccomp-bpf} to perform syscall interposition \cite{opencontainerinitiative}. The rule-based filtering of existing \texttt{seccomp-bpf} in Linux, however, does not allow for more advanced access control policy validation or syscall emulation, although possible approaches to enable more advanced control have been proposed \cite{seccompuserspace}.
...
The output of \texttt{strace} informed us that the \texttt{ps} tool largely performs filesystem operations, since the majority of the system calls utilized are filesystem-related functions (such as \texttt{open}, \texttt{read}, and \texttt{stat}). Inspecting the actual system call arguments to the \texttt{open} function provided clarity to what files are being used. Figure 3 shows truncated output of \texttt{strace} with a filter on the \texttt{open} syscall name. We see that the files being opened are under the \texttt{/proc} filesystem, indicating that \texttt{ps} reads from \texttt{/proc} to determine what processes are running and the state of each process. We discovered similar findings from the programs \texttt{df}, \texttt{free}, \texttt{pidstat}, \texttt{sar}, and \texttt{gkrellm}, with the \texttt{df} tool also requiring access to the \texttt{statfs} system call.
...
We also evaluated GlassBox's fault tolerance by writing programs that try to delete all files on the filesystem and try to send an interrupt signal to all processes listed under \texttt{/proc}. We discuss our findings in Section 6.4.
...
\subsection*{6.4 Fault-Tolerance}
To evaluate the fault-tolerance of GlassBox, we wrote two programs to run inside the container: one program iterates through all files on the filesystem and attempts to remove them; and one program iterates through all PIDs listed under \texttt{/proc} and attempts to send an interrupt signal their respective host process. Due to its read-only mount through \texttt{bindfs},

GlassBox is able to withstand the file deletions, creating errors when the program attempts to delete read-only files and keeping them untouched on the host. Deletions did not create errors under the folders \texttt{/tmp} and \texttt{/home/<user>}, as these directories were mounted with a writeable \texttt{tmpfs} and \texttt{overlayfs} filesystem, respectively. However, this is not an issue for the host, since once the container exited, the \texttt{tmpfs} and \texttt{overlayfs} mounts were discarded, leaving the host untouched. In fact, as GlassBox uses a \texttt{tmpfs} to hold the \texttt{overlayfs} directories, we discovered that for a small enough sized \texttt{tmpfs} the deletions can actually create ``No space left on device'' errors inside the container due to the large number of ``whiteout'' files created by \texttt{overlayfs} to account for the deletions. Again, the host was unaffected by these errors.

GlassBox is also able to withstand sending signals to host processes through its usage of PID namespaces. As processes inside the container run in a separate PID namespace than the host, processes in the host cannot be addressed by PID from inside the container. Our test program failed to send signals to all processes, except for a few. Upon further investigation, we discovered that the few processes the signals were sent to were in fact processes running within the container, but shared a PID with a different process on the host. Therefore, only processes within the container were affected, but all processes running outside of the container were unaffected.

\subsection*{6.5 Evaluation Summary}
...
The evaluation of fault-tolerance using programs to delete files and to send signals to other processes demonstrates that GlassBox provides strong isolation semantics, by protecting the host filesystem and host processes outside of the container.
