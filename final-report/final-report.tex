...
% Need to expand this section to discuss other tools (same tools listed in the results section)
% need to discuss network tool strace
...
Figure 6 shows the output from running `\texttt{ps -a}' inside and outside the container. The container displays \texttt{?} for the TTY since there are no terminals or pseudo-terminals attached to processes outside of the container. Also, processes in the container cannot be seen by the host's \texttt{ps} output. The \texttt{pidstat} discrepancies were also due to this. 
 \begin{figure}[h]
\begin{lstlisting}[linewidth=\linewidth]
  PID TTY          TIME CMD
